...
public class MainProgram extends JPanel implements KeyListener, WindowListener, RobotReturn {
	...
	public MainProgram(LineMap map, Robot robot, int theta2) {
		this.robot = robot;
	...
	private void colectSonar() {
		...
		String rand_name = "theta" + this.theta2 + ".txt";
		...
		for (DataPose d : data) {
			...
			double expected = smodel.expectedSonarRead(d.getPose(), d.getSensorAngle()-90, this.theta2);
	...
	public void robotData(DataPose data) {
		...
		double expected = smodel.expectedSonarRead(p, data.getSensorAngle()-90, this.theta2);
	...
	public static void main(String[] args) {
		...
		Robot robotbt = new BluetoothRobot("NXT10");
		String s = "LCPRobot";
		...
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				new MainProgram(map, (Robot) robot, Integer.parseInt(args[0]));
			}
		});
	}
}
